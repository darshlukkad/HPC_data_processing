...
class ParallelDataStore {
    public:
        ...
        std::vector<const ServiceRequest*> searchByZip(uint32_t zip_min, uint32_t zip_max, int num_threads) const;
        std::vector<const ServiceRequest*> searchByDate(uint32_t from, uint32_t to, int num_threads) const;
